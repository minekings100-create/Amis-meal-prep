      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2 mb-1.5">
          ...
          {review.isVerifiedPurchase && (
            <span className="inline-flex items-center gap-1 text-[10px] font-semibold uppercase tracking-wider text-(--color-brand-yellow)">
        ...
        <div className="flex flex-wrap items-center gap-x-4 gap-y-1 mt-3 text-xs text-stone-500">
          ...
          <span>
            Product:{' '}
            <Link href={`/admin/products?q=${encodeURIComponent(review.productSlug)}`} className="text-stone-700 hover:text-(--color-brand-yellow)">
              {review.productName}
            </Link>
          </span>
        </div>
            ...
            <Dialog.Portal>
              ...
              <Dialog.Content className="fixed z-50 left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 w-[90vw] max-w-md rounded-2xl bg-white shadow-2xl p-6 focus:outline-none">
                ...
                <textarea
                  ...
                  className="w-full px-3 py-2 rounded-md border border-stone-200 text-sm focus:outline-none focus:border-(--color-brand-yellow)"
                />
