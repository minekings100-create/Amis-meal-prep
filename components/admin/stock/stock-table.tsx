    ...
    return (
      <div className="flex items-center gap-1">
        <input
          ...
          min={0}
          className={cn(
            'h-8 w-20 px-2 rounded-md border bg-white text-sm font-mono tabular-nums focus:outline-none focus:ring-2 focus:ring-(--color-brand-yellow-bright)/40',
            error ? 'border-red-300' : 'border-(--color-brand-yellow)',
          )}
        />
  ...
  return (
    <button
      ...
      className={cn(
        ...
        flash && 'bg-(--color-brand-yellow-bright)/30 ring-2 ring-(--color-brand-yellow)',
      )}
    >
      <span className={cn(stock === 0 && 'text-red-600', stock > 0 && stock < 10 && 'text-amber-700')}>
        {stock}
      </span>
      {flash && <Check className="h-3 w-3 text-(--color-brand-yellow)" />}
    </button>
  );
}
...
function StockStatusPill({ stock, isActive }: { stock: number; isActive: boolean }) {
  ...
  return (
    <span className="inline-flex items-center px-2 py-0.5 rounded-full border border-(--color-brand-yellow-bright)/30 bg-(--color-brand-yellow-bright)/15 text-(--color-brand-yellow) text-[10px] font-semibold uppercase tracking-wider">
      In voorraad
    </span>
  );
}
...
function RealtimeIndicator({ status }: { status: RealtimeStatus }) {
  const cfg = {
    ...
    live: { label: 'Live — wijzigingen worden direct gedeeld', dot: 'bg-(--color-brand-yellow-bright) animate-pulse' },
